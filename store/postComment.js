...
export const actions = {
  ...
  async add({ rootState, commit, dispatch }, { postLabel, body }) {
    ...
    if (!result.is_error) {
      ...
      commit("postCommentList/ADD_POST_COMMENT_LIST_DATA", result.data, {
        root: true
      })

      dispatch("flashMessage/showSuccess", "コメント投稿しました。", {
        root: true
      })
    } else {
      // エラー処理
  ...
  async delete(
    ...
    { postCommentLabel, postCommentIndex }
  ) {
    const param = {
    ...
    if (!result.is_error) {
      ...
      commit("postCommentList/DELETE_POST_COMMENT", postCommentIndex, {
        root: true
      })

      dispatch("flashMessage/showSuccess", "コメント削除しました。", {
        root: true
      })
    } else {
      // エラー処理
