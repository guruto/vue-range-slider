...
export const mutations = {
	SET_MEMBER_DATA: function (state, data) {
		...
		this.$cookies.set('AuthorizationToken', data.user.authorization_token, {path: '/', maxAge: 60 * 60 * 24 * 14, domain: process.env.COOKIE_DOMAIN})
	},
...
export const actions = {
	async signUp({ commit, dispatch }, { email, password, pageLabel }) {
		...
		if (!res.is_error) {
			...
			commit("user/SET_USER_MEMBER_PAGE_LABEL_LIST_DATA", res.data.user.member_page_label_list, {root: true})
			
			const message = res.hasOwnProperty('success_message') && res.success_message.length > 0 ? res.success_message : 'メンバー登録完了しました。'
	...
	async signUpCallbackFromTwitter({ commit, dispatch }, { oauthToken, oauthVerifier, tmpOauthToken, tmpOauthTokenSecret, pageLabel }) {
		const params = {
	...
	async signUpForAuthenticated({ commit, dispatch, rootState }, { pageLabel }) {
		...
		const res = await Api.memberSignUpForAuthenticated(params, rootState.user.authorizationToken)
	...
	async loginCallbackFromTwitter({ commit, dispatch }, { oauthToken, oauthVerifier, tmpOauthToken, tmpOauthTokenSecret, pageLabel }) {
		const params = {
			oauth_token:            oauthToken,
			oauth_verifier:         oauthVerifier,
			tmp_oauth_token:        tmpOauthToken,
	...
	async unsubscribe({ commit, rootState, dispatch }, {pageLabel}) {
		...
		const res = await Api.memberUnsubscribe(params, rootState.user.authorizationToken)
		
		if (!res.is_error) {
			dispatch('flashMessage/showSuccess', 'メンバー登録の解除が完了しました。', {root: true})
	...
	async getSetting({ commit, rootState, dispatch }, {pageLabel}) {
		...
		const res = await Api.getMemberSetting(params, rootState.user.authorizationToken)
	...
	async editSetting({rootState, commit, dispatch}, {pageLabel, postNotification}) {
		const params = {
			page_label:        pageLabel,
			post_notification: postNotification ? 1 : 0,
		}
		const result = await Api.editMemberSetting(params, rootState.user.authorizationToken)
	...
	async getProfileMyself({ commit, rootState, dispatch }, {pageLabel}) {
		...
		const res = await Api.getMemberProfileMyself(params, rootState.user.authorizationToken)
	...
	async editProfile({rootState, commit, dispatch}, {pageLabel, name, description, iconImagePath}) {
		const params = {
			page_label:      pageLabel,
			name:            name,
		...
		const result = await Api.editMemberProfile(params, rootState.user.authorizationToken)
