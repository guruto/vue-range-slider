...
export const actions = {
	async get({rootState, commit}, {pageLabel}) {
		const result = await Api.getProfile(pageLabel, {}, rootState.user.authorizationToken)
	...
	async edit({rootState, commit, dispatch}, {name, description, iconImagePath}) {
		// API連携
		const params = {
			// label: label,
			name:            name,
		...
		const result = await Api.editProfile(params, rootState.user.authorizationToken)
