...
export const actions = {
  async getMyself({ rootState, commit }, {}) {
    ...
    if (!result.is_error) {
      result.data.header_media_list = result.data.header_media_list.map(v =>
        Object({ thumbnailImageUrl: v.url, thumbnailImagePath: v.path })
      )
  ...
  async edit(
    ...
    { logo_media_path, header_media_path_list }
  ) {
    // API連携
    const params = {
      type: "header",
    ...
    const result = await Api.editPageDesign(
      params,
