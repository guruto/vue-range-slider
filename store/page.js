...
export const actions = {
	async getWithPostList({rootState, commit}, {label}) {
		...
		if (!pageResult.is_error && !postListResult.is_error && !profileResult.is_error) {
			// pageResult
	...
	async getRequestInfo({rootState, commit}, {pageLabel}) {
		let [pageResult, profileResult, postListResult] = await Promise.all([
			Api.getPage(pageLabel, {}, rootState.user.authorizationToken),
			Api.getProfile(pageLabel, {}, rootState.user.authorizationToken),
			Api.getPostList(pageLabel, {type: 'ANSWER'}, rootState.user.authorizationToken),
		])
		
		if (!profileResult.is_error && !pageResult.is_error && !postListResult.is_error) {
			commit('SET_PAGE_DATA', pageResult.data)
			commit('SET_PAGE_LINK_DATA', pageResult.data.links)
			commit('profile/SET_PROFILE_DATA', profileResult.data, { root: true })
			commit('postList/SET_POST_LIST_DATA', postListResult.data, { root: true })
	...
	async edit({rootState, commit, dispatch}, {label, name, description, twitterId, instagramId, lineId, facebookId}) {
		const params = {
			label:        label,
			name:         name,
	...
	async addLink({rootState, commit, dispatch}, {title, url, orderIndex}) {
		...
		const result = await Api.addPageLink(params, rootState.user.authorizationToken)
	...
	async editLink({rootState, commit, dispatch}, {label, title, url, index}) {
		const params = {
			label: label,
			title: title,
			url:   url,
		}
		const result = await Api.editPageLink(params, rootState.user.authorizationToken)
	...
	async deleteLink({rootState, commit, dispatch}, {label, index}) {
		...
		const result = await Api.deletePageLink(params, rootState.user.authorizationToken)
