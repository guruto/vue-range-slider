...
export const mutations = {
  SET_AUTH_DATA: function(state, authorizationToken) {
    ...
    this.$cookies.set("AuthorizationToken", authorizationToken, {
      path: "/",
      maxAge: 60 * 60 * 24 * 14,
  ...
  SET_USER_DATA: function(state, data) {
    ...
    state.status.isPageDesignModified =
      data.status.is_page_design_modified === "1"
    ...
    this.$cookies.set("AuthorizationToken", data.authorization_token, {
      path: "/",
      maxAge: 60 * 60 * 24 * 14,
      domain: process.env.COOKIE_DOMAIN
    })
...
export const actions = {
  ...
  async signUpCallbackFromTwitter(
    ...
    if (!result.is_error) {
      ...
      const message =
        result.hasOwnProperty("success_message") &&
        result.success_message.length > 0
  ...
  async resetPassword({ commit, dispatch }, { password, reset_password_code }) {
    ...
    if (!result.is_error) {
      ...
      dispatch("flashMessage/showSuccess", "パスワードを再設定しました。", {
        root: true
      })
    } else {
      commit("SET_USER_ERROR", result)
  ...
  async getInfoForDashboard({ commit, state, dispatch }) {
    ...
    if (
      ...
      !notificationListResult.is_error
    ) {
      commit("SET_USER_DATA", result.data)
      commit("page/SET_PAGE_DATA", result.data.page, { root: true })
      commit("profile/SET_PROFILE_DATA", result.data.profile, { root: true })
      commit("postList/SET_POST_LIST_DATA", postListResult.data, { root: true })
      commit("notificationList/SET_DATA", notificationListResult.data, {
        root: true
      })
    } else {
      commit("SET_USER_ERROR", result)
  ...
  async editPassword(
    ...
    { passwordNow, passwordNew, passwordNewConfirm }
  ) {
    const params = {
