...
export const mutations = {
  SET_POST_DATA: function(state, data) {
    ...
    state.publishedAt = data.published_at
      ? moment.unix(data.published_at).format("YYYY/MM/DD HH:mm")
      : ""
...
export const actions = {
  ...
  async modifyDraft({ rootState, commit, dispatch }, { label }) {
    const result = await Api.modifyDraftPost(
      label,
      {},
    ...
    if (!result.is_error) {
      ...
      dispatch("flashMessage/showSuccess", "下書きに戻しました。", {
        root: true
      })
    } else {
      // エラー処理
  ...
  async getMyself({ rootState, commit }, { label }) {
    const result = await Api.getPostMyself(
      label,
      rootState.user.authorizationToken
    )
  ...
  async saveDraftText({ rootState, commit }, param) {
    ...
    const result = await Api.saveTextPost(
      params,
      rootState.user.authorizationToken
    )
  ...
  async uploadImage({ rootState, commit, dispatch }, { fileData }) {
    ...
    const result = await Api.uploadPostImage(
      data,
      rootState.user.authorizationToken
    )
  ...
  async getInfoLinkUrl({ rootState, commit, dispatch }, { linkUrl }) {
    ...
    const result = await Api.getInfoLinkUrl(
      params,
      rootState.user.authorizationToken
    )
  ...
  async uploadFile({ rootState, commit, dispatch }, { fileData }) {
    const data = new FormData()
    data.append("file_data", fileData)
    const result = await Api.uploadPostFile(
      data,
      rootState.user.authorizationToken
    )
