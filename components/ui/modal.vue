		...
		<transition name="c-modal__transition">
			...
			<div class="c-modal__content">
				...
				<div class="c-modal__content__body">
					<div v-if="type == 'description'" class="c-modal__content__body__description">
						<span>{{description}}</span>
					</div>

					<div v-else-if="type == 'page_link'" class="c-modal__content__body__page-link">
						<div class="p-form" style="padding-bottom: 0;">
							<div class="p-form__group">
								<div class="p-form__item">
									<input type="text" placeholder="リンクの名前" :value="pageLinkTitleComputed" @keyup="handleChangePageLinkTitle"/>
								</div>
							</div>
							<div class="p-form__group">
								<div class="p-form__item">
									<input type="text" placeholder="https://pageful.app" :value="pageLinkUrlComputed" @keyup="handleChangePageLinkUrl"/>
								</div>
							</div>
						</div>
					</div>

					<div v-else-if="type == 'add_post'" class="c-modal__content__body__add-post">
						<ul>
							<li @click="hideModal">
							...
							<li @click="hideModal">
								<nuxt-link to="/dashboard/post/add?type=IMAGE">
									<span class="icons icons--image"></span>
									<span>写真・画像</span>
								</nuxt-link>
							</li>
							<li @click="hideModal">
								<nuxt-link to="/dashboard/post/add?type=VIDEO">
									<span class="icons icons--video"></span>
									<span>動画</span>
								</nuxt-link>
							</li>
							<li @click="hideModal">
								<nuxt-link to="/dashboard/post/add?type=SOUND">
									<span class="icons icons--sound"></span>
									<span>音声</span>
								</nuxt-link>
							</li>
							<li @click="hideModal">
								<nuxt-link to="/dashboard/post/add?type=FILE">
									<span class="icons icons--file"></span>
									<span>ファイル</span>
								</nuxt-link>
							</li>
							<li @click="hideModal">
							...
							<li @click="hideModal">
								<nuxt-link to="/dashboard/request">
									<span class="icons icons--answer"></span>
									<span>質問回答</span>
								</nuxt-link>
							</li>
						</ul>
...
<!--							</div>-->
				...
				<div class="c-modal__content__action">
					<div v-if="actionMessage" class="c-modal__content__action__btn-wrapper is-action">
						<button @click="handleButtonAction" class="c-btn">{{actionMessage}}</button>
					</div>
					<div v-if="subActionMessage" class="c-modal__content__action__btn-wrapper is-default">
						<button @click="handleButtonSubAction" class="c-btn">{{subActionMessage}}</button>
					</div>
		...
		methods: {
			...
			handleButtonAction() {
				if (this.type == 'page_link') {
					// urlチェック
					if (!this.pageLinkUrl.match(/^(https?|ftp)(:\/\/[-_.!~*\'()a-zA-Z0-9;\/?:\@&=+\$,%#]+)$/)) {
