<template>
  <div v-if="isActive" class="c-modal">
    ...
    <transition name="c-modal__transition">
      <div
        class="c-modal__content crop-modal"
        :style="{ maxWidth: modalWidth ? modalWidth + 'px' : 'initial' }"
      >
        ...
        <div class="c-modal__content__action">
          ...
          >
            <button class="c-btn c-btn--main" @click="handleButtonAction">
              {{ actionMessage }}
            </button>
          </div>
          <div class="c-modal__content__action__btn-wrapper">
            <button class="c-btn c-btn--default" @click="handleButtonCancel">
              キャンセル
            </button>
          </div>
        </div>
  ...
  methods: {
      ...
      if (image && this.$refs.canvas) {
        ...
        if (!canMoveXR && canMoveXL && canMoveYT && canMoveYB) {
          ...
          ctx.drawImage(
            ...
            this.iw,
            this.ih,
            (image.width * scale - canvas.width) * -1,
            dy,
            dw,
            dh
          )
          ...
          ctx.clearRect(0, 0, cw, ch)
          ctx.drawImage(image, 0, 0, this.iw, this.ih, 0, dy, dw, dh)
        } else if (canMoveXR && canMoveXL && !canMoveYT && canMoveYB) {
          ctx.clearRect(0, 0, cw, ch)
          ctx.drawImage(image, 0, 0, this.iw, this.ih, dx, 0, dw, dh)
        } else if (canMoveXR && canMoveXL && canMoveYT && !canMoveYB) {
          ctx.clearRect(0, 0, cw, ch)
          ctx.drawImage(
            image,
            ...
            this.ih,
            dx,
            (image.height * scale - canvas.height) * -1,
            dw,
            dh
          )
        } else if (canMoveXR && canMoveXL && canMoveYT && canMoveYB) {
          ctx.clearRect(0, 0, cw, ch)
          ctx.drawImage(image, 0, 0, this.iw, this.ih, dx, dy, dw, dh)
        }
        if (isMouseUp) {
          this.ix = ix
          this.iy = iy
    ...
    onMouseUp(e) {
      if (this.isMouseDown) {
        ...
        this.drawImage(
          this.image,
          this.ix +
            (this.sx - (e.pageX || e.changedTouches[0].clientX)) / this.scale,
          this.iy +
            (this.sy - (e.pageY || e.changedTouches[0].clientY)) / this.scale,
          this.scale,
          true
        )
      }
    ...
    async uploadImage(canvas) {
      ...
      if (this.modalName === "postText") {
        result = await Api.uploadPostThumbnailImage(
          data,
        ...
        result = await Api.uploadPageDesignImage(
          data,
          this.$store.state.user.authorizationToken
        )
        ...
        result = await Api.uploadProfileImage(
          data,
          this.$store.state.user.authorizationToken
        )
      }
