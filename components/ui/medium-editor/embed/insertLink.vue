<template>
  <span
    class="btn-toggle p-editor__embed__btn__link"
    @click="handleTextUrlInfoBtn"
  >
    <i class="fas fa-code" />
  </span>
</template>
  ...
  methods: {
    ...
    detectEmbed(e) {
      if (e.keyCode === 13 && this.embedElm) {
        ...
        const url = this.embedElm.innerHTML.replace(
          /<("[^"]*"|'[^']*'|[^'">])*>/g,
          ""
        )
        console.log(url)
    ...
    async renderEmbed(url, elm) {
      ...
      if (
        !url.match(/^(https?|ftp)(:\/\/[-_.!~*\'()a-zA-Z0-9;\/?:\@&=+\$,%#]+)$/)
      ) {
        // 独自にURL判定
      ...
      const result = await Api.getInfoTextLinkUrl(
        params,
