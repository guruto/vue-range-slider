<template>
  <div class="p-post__content__text__body p-post-text-body">
    <div class="p-post-text-body__item" v-for="(bodyItem, i) in textBody" :key="bodyItem.type + i">
      ...
      <div v-else-if="bodyItem.type == 'attaches'" class="p-post-text-body__item__attaches">
        <div>
          <p class="p-post-text-body__item__attaches__title">{{bodyItem.data.title}}</p>
          <p
            class="p-post-text-body__item__attaches__caption"
          >{{formatBytes(bodyItem.data.file.size)}}</p>
        </div>
        <a
          target="_blank"
