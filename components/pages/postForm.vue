<template>
  <div class="p-form">
    <div class="p-form__operation u-cf" style="padding: 0 5%;">
      ...
      <div class="p-form__operation__save">
        ...
        <button
          ...
          type="button"
          :disabled="!canPublishSetting"
          @click="handlePublishSetting"
        >
          公開設定
        </button>
      </div>
    ...
    <div v-if="post.type == 'TEXT'" class="p-form__text">
      <div class="p-form__thumbnail">
        ...
        <div v-else style="padding: 0 5%;">
          <div
            class="p-form__thumbnail__select"
            @click="handleSelectTextThumbnailImage"
          >
            <img
            ...
            >
            <div v-show="isShowTextPopover" class="p-form__thumbnail__select__popover">
              <ul>
                ...
                <li class="p-form__thumbnail__select__popover__item">
                  <a
                    class="p-form__thumbnail__select__popover__item__link"
                    @click.stop="handleSelectTextUserThumbnailImage"
                  >
                    画像をアップロード
                  </a>
                </li>
      ...
      <div style="padding: 0 5%;">
        ...
        <editor
          ...
          :init-data="initData"
          :authorization-token="this.$store.state.user.authorizationToken"
          @save="handleTextSave"
          @ready="onReady"
          @change="onChange"
        />
          ...
          <div v-if="post.itemLink.linkTitle" class="p-ogp">
            ...
            <div class="p-ogp__info">
              <span class="p-ogp__info__title">{{
                post.itemLink.linkTitle
              }}</span>
              <span class="p-ogp__info__description">{{
                post.itemLink.linkDescription
              }}</span>
              <span class="p-ogp__info__site">{{
                post.itemLink.linkSiteName
              }}</span>
            </div>
          </div>
        </div>
    ...
    <form v-else-if="post.type == 'IMAGE'">
      ...
      >
        <div
          v-for="(uploadedPostImage, uploadedPostImageLabel, index) in this
            .$store.state.post.uploadedPostImages"
          :key="index"
          class="p-form__image-upload__content__item"
        >
          <span
    ...
    <form v-else-if="post.type == 'FILE'">
        ...
        <div v-show="post.itemFile.url" class="p-form__group">
          <div class="p-form__item">
            <div class="p-form__upload-file">
              ...
              <p class="p-form__upload-file__text">
                {{ post.itemFile.fileSizeText }}
              </p>
            </div>
          </div>
        </div>
      ...
      <div
        v-show="!post.itemFile.url && isFileUploadRetry"
        class="p-form__file-upload p-form__file-upload--retry"
    ...
    <form v-else-if="post.type == 'ANSWER'">
      <div class="answer" style="padding: 0 5%;">
        <div class="request">
          ...
          <span class="request__time">{{
            post.itemAnswer.request.createdAt
          }}</span>
        </div>
    ...
    <div class="p-stock-image-gallery" v-if="isShowStockImageGallery">
      <div class="p-stock-image-gallery__header">
        <input type="text"
    ...
    <CropModal
      ...
      :canvas-width="1280"
      :canvas-height="670"
      @modalClose="handleCloseCropModal"
    />
  </div>
</template>
...
import ApiFromPixabay from "~/plugins/api-from-pixabay"
...
export default {
  ...
  data() {
    return {
      ...
      // editor-js設定
      initData:
        this.post.itemText.draftBody !== undefined &&
        this.post.itemText.draftBody.hasOwnProperty("blocks") &&
        this.post.itemText.draftBody.blocks.length > 0
          ? this.post.itemText.draftBody
          : {}
    }
  },
  computed: {
  ...
  mounted() {
    ...
    if (this.post.type == "FILE" || this.post.type == "SOUND") {
      if (
        ...
        0 < this.post.thumbnailMediaList.length
      ) {
        ...
        if (
          this.post.thumbnailMediaList.length < this.thumbnailMediaList.length
        ) {
          this.thumbnailMediaList[
            this.post.thumbnailMediaList.length
          ].disabled = false
        }
      }
    }
  ...
  methods: {
    ...
    getParamsForSound() {
      const params = {
        ...
        thumbnail_media_list: this.getParamsThumbnailMediaList(
          this.thumbnailMediaList
        )
      }
      return params
    ...
    handleSelectTextUserThumbnailImage() {
    ...
    handleTextSave(response) {
      ...
      if (!this.$store.state.post.isError) {
        let message = this.post.isPublished
          ? "一時保存しました。"
          : "下書き保存しました。"
    ...
    async handleFocusOutGetInfoLinkUrl() {
      if (
        this.post.itemLink.linkUrl.length === 0 ||
    ...
    async handleSelectListThumbImage(e) {
      ...
      if (0 < files.length) {
        ...
        if (
          parseInt(e.target.dataset.num) + 1 <
          this.thumbnailMediaList.length
        ) {
          const nextTarget = this.thumbnailMediaList[
            parseInt(e.target.dataset.num) + 1
          ]
        ...
        const result = await Api.uploadPostThumbnailImage(
          data,
    ...
    handleDeleteListThumbImage(e) {
      ...
      const tmps = this.thumbnailMediaList
        .filter(v => v.url !== "")
        .map(v => {
          return {
            url: v.url,
