...
<<<<<<< HEAD
	<div class="p-post">
		<section class="c-section">
			...
			<div class="p-post__thumbnail">
				<div v-if="type == 'TEXT' && thumbnailImageUrl">
					<span class="p-post__thumbnail__image" :style="'background-image: url(' + thumbnailImageUrl + ')'"></span>
					<img :src="thumbnailImageUrl">
				</div>
				<div v-else-if="type == 'ANSWER' && thumbnailImageUrl" class="is-answer">
				...
				<div v-else-if="type == 'LINK'" class="p-post__thumbnail__link-ogp">
					<div v-if="linkSiteType == 'YOUTUBE'">
						<div class="p-ogp" v-if="linkTitle">
							<iframe :src="`https://www.youtube.com/embed/${linkUniqueId}`"
							        frameborder="0"
							...
							<a :href="linkUrl" target="_blank">
								<div class="p-ogp__info">
									...
									<span class="p-ogp__info__description">{{linkDescription}}</span>
			...
			<div class="p-post__content">
				...
				<div>
					<div v-if="scope == 'MEMBER' && !isMine && (!isAuthenticated || !isMember)" class="p-post__content__limited">
					...
					<div v-else>
						<div v-if="type == 'TEXT'">
							<div class="p-post__content__text">
								<post-text-body :textBody="(isPublished) ? textBody.blocks : draftTextBody.blocks"></post-text-body>
							</div>
						</div>
						<div v-else-if="type == 'IMAGE'">
							...
							<div class="p-post__content__image-wrap">
								<div v-for="postImage in this.$store.state.post.itemImages" class="p-post__content__image">
						...
						<div v-else-if="type == 'SOUND'">
							...
							<div style="padding: 20px 5% 10px;">
								<audio controls :src="soundUrl" :style="{width:'100%'}">ご使用の中のブラウザが<code>audio</code>要素に対応していません</audio>
							</div>
						</div>
							...
							<div style="padding: 0 5%;">
								<div class="p-post__content__file">
									<div class="p-post__content__file__text-wrap">
										<p class="p-post__content__file__label">{{ fileName }}</p>
										<p class="p-post__content__file__text">{{ fileSizeText }}</p>
									</div>
									<form :action="baseApiUrl + '/post_files/download'" name="downloadform" method="POST">
										<input type="hidden" name="post_label" :value="label">
										<a class="p-post__content__file__button" href="javascript:downloadform.submit()">ダウンロード</a>
									</form>
								</div>
							</div>
						</div>
				...
				<div v-if="type == 'ANSWER' && !isMine" style="padding: 0 5%;">
					<div class="p-user-top__request">
						<a class="c-btn p-user-top__request__action" :href="$getUrl('/request', this.$store.state.page.label)">{{profile.name ? profile.name + 'さんに' : ''}}質問をする</a>
					</div>
				</div>
			...
			<div class="p-post__foot">
				<div class="p-post__foot__share">
					<div class="p-post__foot__share__btn is-twitter">
						<a :href="'http://twitter.com/share?url='+ encodeURIComponent($getUrl('/post/item/' + label, pageLabel)) +'&text=' + encodeURIComponent(comment.length > 127 ? comment.substr(0, 126) + '…' : comment)" target="_blank">
							<img src="/img/share_btn_twitter@2x.png" alt="Twitterでシェアする">
							<span>ツイート</span>
						</a>
					</div>
					<div class="p-post__foot__share__btn is-line">
						<a :href="'https://social-plugins.line.me/lineit/share?url='+ encodeURIComponent($getUrl('/post/item/' + label, pageLabel))" target="_blank">
							<img src="/img/share_btn_line@2x.png" alt="LINEでシェアする">
							<span>LINE</span>
						</a>
					</div>
					<div class="p-post__foot__share__btn is-link-copy">
						<div v-clipboard:copy="$getUrl('/post/item/' + label, pageLabel)" v-clipboard:success="clipboardSuccess" v-clipboard:error="clipboardError">
				...
				<div v-if="isMine" class="p-post__foot__more" @click="handleMoreControlClick">
					<img src="/img/share_btn_more@2x.png" class="is-more-control">
					<div class="p-popover" v-show="isShowPopoverMoreControl">
						<div class="p-popover__arrow"></div>
						<div class="p-popover__body">
							<ul>
								<li><nuxt-link :to="`/dashboard/post/edit/${label}`">編集する</nuxt-link></li>

								<li v-if="isPublished" @click="handleModifyDraft">下書きに戻す</li>
								<li v-else @click="handleDeleteClick" class="is-delete">削除する</li>
							</ul>
						</div>
					</div>
				</div>
			...
			<div class="p-post__comment">
				<h2 class="c-title--sub">コメント{{(isMine || isMember) ? '（' + this.$store.state.postCommentList.itemCount + '件）' : '' }}</h2>

				<div v-if="isMine || isMember">
					<div v-if="this.$store.state.postCommentList.itemCount > 0" class="p-post__comment__list">
						<div v-for="(postComment, postCommentIndex) in this.$store.state.postCommentList.items" class="p-post__comment__list__item">
							<div class="p-post__comment__list__item__profile">
								<div class="p-post__comment__list__item__profile__icon">
									<span class="p-post__comment__list__item__profile__icon__image" :style="'background-image: url(' + postComment.profile.icon_image_url + ')'"></span>
								</div>
								<div class="p-post__comment__list__item__profile__name">
									<span class="is-name">{{postComment.profile.name}}</span>
									<span class="is-time">{{postComment.created_at | moment}}</span>
								</div>
							</div>
							<div v-if="isMine || postComment.is_mine    " @click="handleControlPostComment" :data-post-comment-label="postComment.label" :data-post-comment-index="postCommentIndex" class="p-post__comment__list__item__delete">
								<img src="/img/share_btn_more@2x.png" class="is-post-comment-control">
							</div>
					...
					<div v-else>
						<p style="margin-bottom: 15px;">コメントはまだありません。</p>
					</div>

					<div class="p-post__comment__add">
						<div class="p-post__comment__add__body">
							<div class="p-form">
								<textarea v-model="commentBody" rows="4" placeholder="コメントを入力">{{commentBody}}</textarea>
							</div>
						</div>
						<div class="p-post__comment__add__action">
							<button @click="handleAddPostComment" type="button" :class="{'c-btn': true, 'c-btn--main': true, 'is-loading': isAddCommentLoading}" :disabled="isAddCommentLoading">コメント投稿</button>
						</div>
					</div>
				</div>
				<div v-else>
					<p style="text-align: center;margin-bottom: 15px;">メンバーになるとコメントを閲覧、投稿することができます。</p>
			...
			<div v-if="isUserPage" class="p-post__profile">
				<div class="p-post__profile__head">
					<div class="p-post__profile__thumbnail">
						<span class="p-post__profile__thumbnail__image" :style="'background-image: url(' + profile.iconImageUrl + ')'"></span>
					</div>
			...
			<div class="p-post__related">
				...
				<post-list
					:posts="relatedPosts"
					:isDashboard="!isUserPage"></post-list>
			</div>

			<div class="u-mt-24 u-ta-c">
				<nuxt-link :to="this.$store.state.subDomain.hasSubDomain ? '/' : '/dashboard/post/manage'" class="c-btn c-btn--default">{{this.$store.state.subDomain.hasSubDomain ? 'ホームに戻る' : '投稿管理に戻る'}}</nuxt-link>
			</div>
		...
		<div class="p-popover" id="post-comment-popover" v-show="isShowPopoverControlPostComment">
			...
			<div class="p-popover__body">
				<ul>
					<li @click="handleDeletePostComment" class="is-delete">削除する</li>
				</ul>
			</div>
		</div>
