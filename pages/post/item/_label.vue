  ...
  <div class="p-post">
    <section class="c-section">
      ...
      >
        <span
          v-if="isMine"
          :class="{'p-post__label__publish': true, 'is-published': isPublished}"
        >{{isPublished ? '公開中' : '下書き'}}</span>
        <span
          v-if="isMine"
          :class="{'p-post__label__scope': true}"
      ...
      <div class="p-post__content">
        ...
        <div>
          <div
            v-if="scope == 'MEMBER' && !isMine && (!isAuthenticated || !isMember)"
      ...
      <div class="p-post__foot">
        <div class="p-post__foot__share">
          <div class="p-post__foot__share__btn is-twitter">
            <a
              :href="'http://twitter.com/share?url='+ encodeURIComponent($getUrl('/post/item/' + label, pageLabel)) +'&text=' + encodeURIComponent(comment.length > 127 ? comment.substr(0, 126) + '…' : comment)"
              target="_blank"
            >
          ...
          <div class="p-post__foot__share__btn is-line">
            <a
              :href="'https://social-plugins.line.me/lineit/share?url='+ encodeURIComponent($getUrl('/post/item/' + label, pageLabel))"
              target="_blank"
            >
        ...
        <div v-if="isMine" class="p-post__foot__more" @click="handleMoreControlClick">
          ...
          <div class="p-popover" v-show="isShowPopoverMoreControl">
            ...
            <div class="p-popover__body">
              <ul>
                <li>
                  <nuxt-link :to="`/dashboard/post/edit/${label}`">編集する</nuxt-link>
                </li>
      ...
      <div class="p-post__comment">
        <h2
          class="c-title--sub"
        >コメント{{(isMine || isMember) ? '（' + this.$store.state.postCommentList.itemCount + '件）' : '' }}</h2>

        <div v-if="isMine || isMember">
          <div v-if="this.$store.state.postCommentList.itemCount > 0" class="p-post__comment__list">
            <div
              v-for="(postComment, postCommentIndex) in this.$store.state.postCommentList.items"
              ...
              <div class="p-post__comment__list__item__profile">
                <div class="p-post__comment__list__item__profile__icon">
                  <span
                    ...
                    :style="'background-image: url(' + postComment.profile.icon_image_url + ')'"
                  ></span>
                </div>
          ...
          <div class="p-post__comment__add">
            ...
            <div class="p-post__comment__add__action">
              <button
                ...
                type="button"
                :class="{'c-btn': true, 'c-btn--main': true, 'is-loading': isAddCommentLoading}"
      ...
      <div class="p-post__related">
        ...
        <post-list :posts="relatedPosts" :isDashboard="!isUserPage"></post-list>
      </div>

      <div class="u-mt-24 u-ta-c">
        <nuxt-link
          :to="this.$store.state.subDomain.hasSubDomain ? '/' : '/dashboard/post/manage'"
          class="c-btn c-btn--default"
        >{{this.$store.state.subDomain.hasSubDomain ? 'ホームに戻る' : '投稿管理に戻る'}}</nuxt-link>
      </div>
    </section>
