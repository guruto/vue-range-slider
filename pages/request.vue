	...
	<div class="p-request">
		<section class="c-section">
				...
				<div class="p-user-top">
					<div class="p-user-top__head">
						<div class="p-user-top__head__title">
							<img v-if="this.$store.state.page.design.logoMediaUrl !== ''"
								class="logo_image"
								:src='this.$store.state.page.design.logoMediaUrl' :alt="page.name">
			...
			<div class="p-request__content">
				<h1 class="c-title" style="text-align: center; font-size: 16px">{{profile.name}}さんに質問</h1>
				<p style="text-align: center; margin-bottom: 20px;">聞きたいことを匿名で質問できます。</p>

				<div class="p-form">
					<form @submit.prevent="addRequest">
						<div class="p-form__group">
							<div class="p-form__item">
								<textarea v-model="requestBody" rows="8" placeholder="質問内容を入力"></textarea>
							</div>
						</div>

						<div class="p-form__button">
							<button :class="{'c-btn': true, 'c-btn--main': true, 'is-loading': isLoading}" type="submit" :disabled="isLoading">質問する</button>
						</div>
					</form>
				</div>
			</div>
			...
			<div v-if="postList.length > 0" class="p-request__post">
				<h2 class="c-title--sub">過去の回答</h2>
				<div v-for="item in postList" class="p-request__post__item">
					<nuxt-link :to="'/post/item/' + item.label">
						<div v-if="item.thumbnail_image_url" class="p-request__post__item__thumbnail">
							<img v-if="item.thumbnail_image_url" :src="item.thumbnail_image_url">
						</div>
						...
						<div class="u-cf" style="padding: 4px 16px 16px">
							<span class="p-request__post__item__time">{{item.published_at | moment}}</span>
						</div>
					</nuxt-link>
				</div>
			</div>
			...
			<div class="u-mt-32 u-ta-c">
				<nuxt-link to="/" class="c-btn c-btn--default">ホームに戻る</nuxt-link>
			</div>
		...
		async asyncData(context) {
			...
			if (context.store.state.page.isError) {
				...
				return {
					...
					meta: {
						title: (context.store.state.profile.name) ? context.store.state.profile.name + 'さんへの質問募集' : '質問募集',
						twitterTitle: '質問募集',
						description: (context.store.state.profile.name) ? context.store.state.profile.name + 'さんに聞きたいことを匿名で質問できます。' : '聞きたいことを匿名で質問できます。',
