			...
			<div class="p-form">
				<div class="p-form__group">
					...
					<div class="p-form__item u-cf">
						<span class="p-form__value" style="vertical-align: -13px;">{{info.TWITTER.linked == '1' ? '@' + info.TWITTER.screen_name : '未連携'}}</span>
						<button @click="handleTwitter" type="button" :class="{'c-btn': true, 'is-loading': isLoading, 'c-btn--main': info.TWITTER.linked != '1', 'c-btn--default': info.TWITTER.linked == '1'}" :disabled="isLoading" style="float: right;">{{info.TWITTER.linked == '1' ? '解除' : '連携する'}}</button>
					</div>
				</div>
			</div>
		</section>
		...
		methods: {
			async handleTwitter() {
				...
				if (this.info.TWITTER.linked == '0') {
					// 連携処理
					const result = await Api.getRedirectDataWithSocial({type: 'TWITTER', method: 'link'})
