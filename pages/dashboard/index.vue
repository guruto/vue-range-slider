<template>
  <div class="p-dashboard">
    ...
    <div style="padding: 0 5% 30px;">
      <a
        class="c-btn c-btn--main p-dashboard__site-link"
    ...
    <div class="p-dashboard__attention">
      ...
      >
        ...
        <div class="p-dashboard__attention__item__content">
          ...
          <p class="p-dashboard__attention__item__content__description">
            ブログ、画像、動画、ファイル、URL引用など様々な形式で投稿を作成できます。
          </p>
        </div>
      </div>
      <div
        v-if="this.$store.state.user.status.requestCount == 0"
        class="p-dashboard__attention__item"
      >
        <div
          class="p-dashboard__attention__item__icon"
        ...
        <div class="p-dashboard__attention__item__content">
          ...
          <p class="p-dashboard__attention__item__content__description">
            Twitterなどで質問を集めて回答したり、書いてほしいブログテーマを集めることができます。
          </p>
        </div>
        <nuxt-link to="/dashboard/request" />
      </div>
      <div
        v-if="!this.$store.state.user.status.isPageDesignModified"
        class="p-dashboard__attention__item"
      >
        <div
          class="p-dashboard__attention__item__icon"
        ...
        <div class="p-dashboard__attention__item__content">
          ...
          <p class="p-dashboard__attention__item__content__description">
            ヘッダー画像、ロゴ画像を設定することができます。
          </p>
        </div>
        <nuxt-link to="/dashboard/setting/design" />
      </div>
      <div
        v-if="!this.$store.state.user.status.isEmailVerified"
        class="p-dashboard__attention__item"
      >
        <div
          class="p-dashboard__attention__item__icon"
        ...
        <div class="p-dashboard__attention__item__content">
          ...
          <p class="p-dashboard__attention__item__content__description">
            届いている認証メールをご確認ください。再度認証メールを送信する場合はこちらをクリックしてください。
          </p>
        </div>
        <nuxt-link to="/dashboard/user/resend_verify_email" />
      </div>
    </div>
    ...
    <section class="c-section">
      <div class="p-dashboard__group">
        ...
        <div class="p-dashboard__group__item">
          ...
          <p class="p-dashboard__group__item__value">
            現在の投稿数 {{ this.$store.state.user.status.postCount }}
          </p>
        </div>
        <div class="p-dashboard__group__item">
          ...
          <p
            ...
            class="p-dashboard__group__item__value"
          >
            投稿はまだありません
          </p>
          ...
          <div v-if="this.$store.state.postList.items.length > 0" class="post">
            <div
              v-for="(post, postIndex) in this.$store.state.postList.items"
              :key="postIndex"
              class="post__item"
            >
              <nuxt-link :to="'/post/item/' + post.label">
                <div class="post__item__content">
                  <div class="post__item__thumbnail">
                    <span
                      v-if="!post.post_item.thumbnail_image_url"
                      class="post__item__thumbnail__image"
                      :style="
                    ...
                    <span
                      ...
                      class="post__item__thumbnail__image"
                      :style="
                        'background-image: url(' + post.post_item.url + ')'
                      "
                    />
                  </div>
                  <div class="post__item__title">
                    <span class="is-title">{{ post.title }}</span>
                    <span class="is-comment">{{
                      post.comment.length > 50
                        ? post.comment.substr(0, 50) + "…"
                        : post.comment
                    }}</span>
                  </div>
                </div>
                <div class="post__item__info u-cf">
                  <div style="float: left">
                    <div
                      :class="{
                        ...
                        'is-published': post.is_published === '1'
                      }"
                    >
                      <span>{{
                        post.is_published === "1" ? "公開中" : "下書き"
                      }}</span>
                    </div>
                  ...
                  <span class="post__item__info__time">{{
        ...
        <div class="p-dashboard__group__action">
          <nuxt-link to="/dashboard/post/manage" class="c-btn">
            もっと見る
          </nuxt-link>
        </div>
      </div>
