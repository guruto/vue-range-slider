      ...
      <div class="p-request-list">
        <div class="p-request-list__head">
          ...
          <button
            ...
            class="p-request-list__head__item is-copy c-btn"
          >
            質問募集リンクをコピーする
          </button>
        </div>
        ...
        <div class="p-request-list__tab">
          <div
            :class="{
              'p-request-list__tab__item': true,
              'is-selected': selectType == 'ALL'
            }"
          >
            <nuxt-link to="/dashboard/request?select_type=ALL">
              全て
            </nuxt-link>
          </div>
          <div
            :class="{
              'p-request-list__tab__item': true,
              'is-selected': selectType == 'NOT_ANSWERED'
            }"
          >
            <nuxt-link to="/dashboard/request?select_type=NOT_ANSWERED">
              未回答
            </nuxt-link>
          </div>
          <div
            :class="{
              'p-request-list__tab__item': true,
              'is-selected': selectType == 'ANSWERED'
            }"
          >
            <nuxt-link to="/dashboard/request?select_type=ANSWERED">
              回答済み
            </nuxt-link>
          </div>
        </div>
        ...
        <div v-if="requestList.length > 0">
          <div v-for="item in requestList" class="p-request-list__item">
            <nuxt-link
              :to="
                item.is_answered == '1'
                  ? '/post/item/' + item.post.label
                  : '/dashboard/post/add?type=ANSWER&request_label=' +
                    item.label
              "
            >
              ...
              <div
                :class="{
                  'p-request-list__item__answered': true,
                  'is-answered': item.is_answered == '1'
                }"
              >
                <span>{{
                  item.is_answered == "1" ? "回答済み" : "未回答"
                }}</span>
              </div>
              <div class="p-request-list__item__foot u-cf">
                <span class="p-request-list__item__foot__time">{{
                  item.created_at
                }}</span>
                <span
...
export default {
  ...
  computed: {
    shareTweetUrl() {
      ...
      return (
        ...
        encodeURIComponent(
          this.$getUrl("/request", this.$store.state.page.label)
        ) +
        "&text=" +
  ...
  watch: {
    $route(to, from) {
      // ルートの変更の検知...
      this.selectType = this.$route.query.select_type
  ...
  async asyncData(context) {
    ...
    const selectType = context.route.query.select_type
      ? context.route.query.select_type
      : "ALL"

    await context.store.dispatch("requestList/getListMyself", {
      type: "QUESTION",
  ...
  methods: {
    ...
    async getRequestListSelectType(selectType) {
      ...
      await this.$store.dispatch("requestList/getListMyself", {
        type: "QUESTION",
        selectType: selectType
      })
