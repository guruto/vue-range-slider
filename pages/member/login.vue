<template>
	<div class="p-member-login">
		...
		<section class="c-section">
			<h1 class="c-title u-ta-c">メンバーログイン</h1>
			<p style="text-align: center;">「{{page.name}}」にログイン。</p>

			<div class="p-login">
				<div class="p-login__service">
					<div class="p-login__service__item">
						<button @click="handleMemberLoginTwitter" type="button" :class="{'c-btn': true, 'is-twitter': true, 'is-loading': isLoadingLoginTwitter}"><img src="/img/icon_service_twitter@2x.png" alt="Twitterでメンバーログイン">Twitterでメンバーログイン</button>
					</div>
					<div class="p-login__service__separator">もしくは</div>
				</div>

				<div class="p-login__form">
					<h2 class="c-title--sub">メールアドレスでメンバーログイン</h2>
					<div class="p-form">
						<form @submit.prevent="login">
							...
							<div class="p-form__button">
								<button :class="{'c-btn': true, 'c-btn--main': true, 'is-loading': isLoading}" type="submit" :disabled="isLoading">メンバーログイン</button>
							</div>
							<div class="p-form__button">
								<nuxt-link class="c-btn c-btn--default" to="/member/sign_up">未登録の場合はメンバー登録へ</nuxt-link>
							</div>
						</form>
					</div>
				</div>
	...
	export default {
		...
		methods: {
			async handleMemberLoginTwitter() {
				...
				const result = await Api.getRedirectDataWithSocial({type: 'TWITTER', method: 'member_login', page_label: this.page.label})
