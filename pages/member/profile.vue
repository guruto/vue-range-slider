					...
					<form @submit.prevent="update">
						<div class="p-form__group">
							...
							<div class="p-form__label__spinner">
								<spinner :isActive="isIconUploading"></spinner>
							</div>
							<div class="p-form__image">
								<div class="p-form__image__content">
									<span class="p-form__image__content__data" v-if="iconImageUrl" :style="'background-image: url(' + iconImageUrl + ')'"></span>
									...
									<input type="file" id="form-select-icon-image" @change="handleInputIconImageFile"/>
								</div>
							</div>
						</div>
	...
	export default {
		...
		methods: {
			...
			async uploadImage(selectedImage, type) {
				...
				const result = await Api.uploadMemberProfileImage(data, this.$store.state.user.authorizationToken)
