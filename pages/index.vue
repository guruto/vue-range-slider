		...
		<section v-if="this.$store.state.subDomain.hasSubDomain" class="c-section">
			<div class="p-user-top">
				...
				<div class="p-user-top__request">
					<nuxt-link class="c-btn p-user-top__request__action" to="/request">{{profile.name ? profile.name + 'さんに' : ''}}質問をする</nuxt-link>
				</div>

				<post-list
					:posts="this.$store.state.postList.items"
					:isDashboard="false"></post-list>

				<div v-if="this.$store.state.postList.items.length > 0" class="p-user-top__request u-mt-40">
					<nuxt-link class="c-btn p-user-top__request__action" to="/request">{{profile.name ? profile.name + 'さんに' : ''}}質問をする</nuxt-link>
				</div>
...
export default {
	...
	data() {
		return {
			...
			meta: {
				title: 'PAGEFUL（ペイジフル） - 誰でも簡単にコンテンツ配信・販売サイトを作成',
				notUseBaseTitle: true,
				description: 'PAGEFUL（ペイジフル）では、ブログ、写真・画像、URLなど様々な形式でコンテンツを作成・配信できます。ブログ、自分のホームページ、思い出の写真のストック場所など用途も自由です。',
	...
	async asyncData(context) {
		...
		if (pageLabel) {
			...
			if (context.route.query.hasOwnProperty('success_message') && context.route.query.success_message.length > 0) {
				context.store.dispatch('flashMessage/showSuccess', context.route.query.success_message);
			}
			...
			return {
				...
				meta: {
					...
					notUseBaseTitle: true,
					description:     context.store.state.page.description ? context.store.state.page.description : context.store.state.page.name,
