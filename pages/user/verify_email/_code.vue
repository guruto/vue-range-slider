<template>
	<div>
		<h1 class="c-title">メールアドレスの認証結果</h1>
		<section class="c-section">
			<div class="p-verify-email">

				<div v-if="isVerified">
					<h2 class="c-title--sub">認証に成功しました</h2>
					<p>メールアドレスの認証が完了しました。</p>

					<div class="p-verify-email__login">
						<nuxt-link to="/dashboard" class="c-btn c-btn--main">ログイン</nuxt-link>
					</div>
				</div>

				<div v-else>
					<div v-if="this.$store.state.user.errorRedirectPath == '/user/login'">
						<h2 class="c-title--sub">{{this.$store.state.user.errorMessage}}</h2>

						<p>既にメールアドレスの認証が完了していますので、ログインをお願いします。</p>

						<div class="p-verify-email__login">
							<nuxt-link to="/user/login" class="c-btn c-btn--main">ログイン</nuxt-link>
						</div>
					</div>

					<div v-else>
						<h2 class="c-title--sub">認証に失敗しました</h2>
						<p>メールに届いた認証URLを正しくクリックできているかご確認ください。</p>

						<div class="u-hidden">
							<p>それでも失敗する場合は、登録されたメールアドレスへ再度認証メールをお送りいたします。</p>
	...
	export default {
		...
		data() {
			return {
				...
				meta: {
					title: 'メールアドレスの認証結果',
					description: 'PAGEFUL（ペイジフル）のメールアドレスの認証結果の確認ページ',
				}
			}
		},
