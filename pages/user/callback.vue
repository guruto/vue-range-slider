		...
		async mounted() {
			...
			if (method == 'member_sign_up' || method == 'member_login') {
				...
				if (this.$store.state.member.isError) {
					...
					window.location.href = url + '?error_message=' + this.$store.state.member.errorMessage
					return
					...
					if (method == 'member_sign_up') {
						...
						path = '/'
						message = this.$store.state.member.successMessage.length > 0 ? this.$store.state.member.successMessage : 'メンバー登録完了しました。'
